...
def execute(curs=None):
    ...
    curs.execute(PostgreSQLQuery.from_(queue_table).
                 select('stage_queue_id', 'job_status_id').
                 where(queue_table.started == False).
                 orderby('stage_queue_id').get_sql())
    fetch_result = curs.fetchone()
    if fetch_result is not None:
        stage_queue_id, job_status_id = fetch_result
        curs.execute(PostgreSQLQuery.update(queue_table).
                     set(queue_table.started, True).
                     where(queue_table.stage_queue_id == stage_queue_id).get_sql())
